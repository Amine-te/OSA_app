...
class GradientHeader(QWidget):
    """Compact control-center strip with pipeline state (minimal vertical use)."""

    def __init__(self, title="OSA Control Center", subtitle="", parent=None):
        super().__init__(parent)
        self.title = title
        self.subtitle = subtitle  # kept for API; not drawn when empty
        self.state_text = "IDLE"
        self.workspace_text = ""
        self.setFixedHeight(46)
        self.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)

    def set_state(self, state: str, workspace: str = ""):
        self.state_text = state
        self.workspace_text = workspace or ""
        self.update()
...
class LogConsole(QWidget):
    """Dock-style log with level filter, search, and copy."""

    def __init__(self, parent=None):
        super().__init__(parent)
        from PyQt6.QtWidgets import QTextEdit, QComboBox, QLineEdit

        self._entries = []  # list of dict: level, html line, plain
        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(0)

        # Toolbar (combo/search need explicit QSS: global theme uses tall padding that
        # clips badly when the row is compressed inside a dock.)
        tool = QHBoxLayout()
        tool.setContentsMargins(6, 4, 6, 4)
        tool.setSpacing(8)

        self.header_btn = QPushButton("▼")
        self.header_btn.setFixedSize(24, 26)
        self.header_btn.setToolTip("Show / hide log text")
        ...
        tool.addWidget(self.header_btn)
        ...
        self.log_area.setMinimumHeight(52)
    ...
    def toggle(self):
        ...
        self.header_btn.setText("▼" if self._expanded else "▶")
    ...
    def _apply_filter(self):
        ...
        for e in self._entries:
            if lvl != "ALL" and e["level"].upper() != lvl:
                continue
            plain = e["plain"]
            if q and q not in plain.lower():
                continue
            self.log_area.append(e["html"])
    ...
    def log(self, message, level="info"):
        ...
        plain = f"[{ts}] {level.upper():7} {message}"
        self._entries.append({"level": level, "html": html, "plain": plain})
        if len(self._entries) > 5000:
            self._entries = self._entries[-4000:]
        self._apply_filter()
