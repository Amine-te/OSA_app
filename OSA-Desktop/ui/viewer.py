...
class SplitCompareViewer(QWidget):
    ...
    roi_list_changed = pyqtSignal(list)   # list of [x1,y1,x2,y2]
    ...
    def __init__(self, parent=None):
        ...
        self._roi_list = []  # list of QRectF image coords

        # Compare layout
        self._compare_mode = CompareMode.SLIDER
        self._toggle_show_raw = True  # for TOGGLE_AB

        # Focus mode: dim non-selected boxes
    ...
    def flip_ab_toggle(self):
        if self._compare_mode == CompareMode.TOGGLE_AB:
            self._toggle_show_raw = not self._toggle_show_raw
            self.update()
    ...
    def set_roi_presets(self, rects: list):
        """rects: list of [x1,y1,x2,y2]"""
        self._roi_list = [QRectF(float(r[0]), float(r[1]), float(r[2] - r[0]), float(r[3] - r[1]))
                          for r in rects]
        self.update()
    ...
    def paintEvent(self, event):
        ...
        # ── Draw images (compare modes) ──
        if self._compare_mode == CompareMode.TOGGLE_AB and has_both:
            pm = self._raw_pixmap if self._toggle_show_raw else self._ann_pixmap
            if pm:
                painter.save()
                painter.translate(o)
                painter.scale(s, s)
                ...
                painter.restore()
        elif self._compare_mode == CompareMode.SIDE_BY_SIDE and has_both:
            half_w = self.width() // 2
            for i, pm in enumerate((self._raw_pixmap, self._ann_pixmap)):
                painter.save()
                painter.setClipRect(half_w * i, 0, half_w, self.height())
                painter.translate(o)
                painter.scale(s, s)
                painter.drawPixmap(0, 0, pm)
                painter.restore()
        ...
        if self._detections and not self._heatmap_enabled:
            # When comparing raw vs annotated, only draw detections on the annotated side.
            draw_dets = True
            if has_both:
                if self._compare_mode == CompareMode.TOGGLE_AB:
                    draw_dets = not self._toggle_show_raw
                    if draw_dets:
                        painter.save()
                elif self._compare_mode == CompareMode.SIDE_BY_SIDE:
                    painter.save()
                    painter.setClipRect(self.width() // 2, 0, self.width() // 2, self.height())
                else:
                    # SLIDER
                    painter.save()
            ...
            if has_both and self._compare_mode in (CompareMode.SLIDER, CompareMode.SIDE_BY_SIDE) and painter.isActive():
                painter.restore()
            if has_both and self._compare_mode == CompareMode.TOGGLE_AB and draw_dets and painter.isActive():
                painter.restore()

        # ── ROI (in-progress + saved) ──
        for rect in list(self._roi_list) + ([self._roi_rect] if self._roi_rect else []):
            if rect is None:
                continue
            tl = self._to_widget(QPointF(rect.x(), rect.y()))
            br = self._to_widget(QPointF(rect.right(), rect.bottom()))
            pen = QPen(QColor(COLORS["info"]), 2, Qt.PenStyle.DashLine)
            painter.setPen(pen)
            painter.setBrush(QColor(88, 166, 255, 30))
            painter.drawRect(QRectF(tl, br))

        # ── Divider (slider mode only) ──
        if has_both and self._compare_mode == CompareMode.SLIDER:
    ...
    def _draw_detection_widget(self, painter, idx, det, scale):
        ...
        dimmed = self._focus_mode and self._highlighted_idx >= 0 and idx != self._highlighted_idx
        ...
        if is_hl or is_fl:
            ...
            fill = QColor(color.red(), color.green(), color.blue(), 8)
        else:
            pw = 2
    ...
    def _generate_heatmap(self):
        """
        Semantic overlay: red channel emphasis for voids (missing), green for detections.
        Alpha scales with model confidence where available.
        """
        pm = self._raw_pixmap or self._ann_pixmap
        if not pm:
            self._heatmap_pixmap = None
        ...
        # Missing / void — red
        for void in self._void_detections or []:
            x1, y1, x2, y2 = [max(0, int(v)) for v in void['bbox']]
            x2, y2 = min(x2, w), min(y2, h)
            conf = float(void.get('combined_confidence', void.get('confidence', 0.5)))
            a = int(80 + 120 * max(0.0, min(1.0, conf)))
            rgba[y1:y2, x1:x2, 0] = np.maximum(rgba[y1:y2, x1:x2, 0], 220)
            rgba[y1:y2, x1:x2, 3] = np.maximum(rgba[y1:y2, x1:x2, 3], a)

        # Detected products — green, opacity ∝ confidence
        for det in self._detections or []:
            x1, y1, x2, y2 = [max(0, int(v)) for v in det['bbox']]
            x2, y2 = min(x2, w), min(y2, h)
            conf = float(det.get('combined_confidence', det.get('confidence', 0.5)))
            a = int(60 + 165 * max(0.0, min(1.0, conf)))
            rgba[y1:y2, x1:x2, 1] = np.maximum(rgba[y1:y2, x1:x2, 1], 200)
            rgba[y1:y2, x1:x2, 3] = np.maximum(rgba[y1:y2, x1:x2, 3], a)
        ...
        ks = max(15, int(min(w, h) * 0.06)) | 1
        for c in range(4):
            rgba[:, :, c] = cv2.GaussianBlur(rgba[:, :, c], (ks, ks), 0)
    ...
    def mousePressEvent(self, event):
        ...
        if (self._compare_mode == CompareMode.SLIDER and self._split_enabled and
                self._raw_pixmap and self._ann_pixmap and abs(pos.x() - dx) < 16):
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._drawing_roi:
            ...
            if self._roi_rect and self._roi_rect.width() > 5 and self._roi_rect.height() > 5:
                box = [int(self._roi_rect.x()), int(self._roi_rect.y()),
                       int(self._roi_rect.right()), int(self._roi_rect.bottom())]
                self._roi_list.append(QRectF(self._roi_rect))
                self._roi_rect = None
                self.roi_drawn.emit(box)
                self.roi_list_changed.emit([[int(r.x()), int(r.y()), int(r.right()), int(r.bottom())]
